...
mod1<-gam(log(flow_out+1)~s(ted,bs='cr')+s(r3score,bs='cr')+s(log(gdp),bs='cr')+s(year,bs='re')+s(country,bs='re'),data=data)
summary(mod1) #0.82
plot(mod1)
#fa cagare :) tentiamo pca
...
for (c in unique(data$country)){
  flow_vec<-data$flow_out[which(data$country==c)]
...
summary(p) 
p$loadings #first comp explains 0.79 of total var, it is a mean -> we can use a mean of data over years 
...
data_sum$flow_out_mean<-apply(data_pca_norm[,-1],1,mean) #flow in

data_sum$flow_out_mean<-data_sum$flow_out_mean*mean_std+mean_m
...
colnames(data_mean)<-c("country","flow_out_mean")
...
mod2<-gam(flow_out_mean~s(ted,bs="cr")+s(score,bs="cr")+s(gdp,bs="cr"),data=data_mean)
summary(mod2) #0.965
shapiro.test(mod2$residuals) #not normal
plot(mod2)

#

data_mean$r3score<-(data_mean$score)^(1/3)
mod3<-gam(log(flow_out_mean+1)~s(ted,bs="cr")+s(r3score,bs="cr")+s(log(gdp),bs="cr"),data=data_mean)
summary(mod3) #0.973
shapiro.test(mod3$residuals) #not normal
plot(mod3)

#no gdp

mod4_full<-gam(log(flow_out_mean+1)~s(ted,bs="cr")+s(r3score,bs="cr"),data=data_mean)
summary(mod4_full) #0.95
shapiro.test(mod4_full$residuals) #normal
plot(mod4_full)

mod4<-gam(log(flow_out_mean+1)~s(ted,bs="cr")+s(r3score,bs="cr",k=6),data=data_mean)
summary(mod4) #0.9
shapiro.test(mod4$residuals) #normal
plot(mod4)

anova(mod4,mod4_full,test="F")
#p=0.008367 we lose something 
...
mod_edu<-gam(log(flow_out_mean)~s(edu_index,bs="cr"),data=data_mean)
summary(mod_edu) #rsq 0.168
shapiro.test(mod_edu$residuals) #normal
plot(mod_edu)

mod_edu_l<-lm(log(flow_out_mean)~edu_index,data=data_mean)
summary(mod_edu_l) #rsq 0.1957
shapiro.test(mod_edu_l$residuals)
plot(mod_edu_l)
anova(mod_edu_l,mod_edu,test="F") # ?
plot(data_mean$edu_index,log(data_mean$flow_out_mean))

mod_uni<-gam(log(flow_out_mean)~s(r3score,bs="cr"),data=data_mean)
summary(mod_uni) #rsq 0.759
shapiro.test(mod_uni$residuals) #normal
plot(mod_uni)

mod_ted<-gam(log(flow_out_mean)~s(ted,bs="cr"),data=data_mean)
summary(mod_ted) #rsq 0.787
shapiro.test(mod_ted$residuals) #normal
plot(mod_ted)

mod_ted_l<-lm(log(flow_out_mean)~ted,data=data_mean)
summary(mod_ted_l) #rsq 0.72
shapiro.test(mod_ted_l$residuals) #normal
plot(mod_ted_l)

anova(mod_ted_l,mod_ted,test="F") #p=0.05

mod_gdp<-gam(log(flow_out_mean)~s(log(gdp),bs="cr"),data=data_mean)
summary(mod_gdp) #0.21
shapiro.test(mod_gdp$residuals) #normal
plot(mod_gdp)

mod_uni_ted<-gam(log(flow_out_mean)~s(r3score,bs="cr")+s(ted,bs="cr"),data=data_mean)
summary(mod_uni_ted) #0.95  
...
mod_uni_gdp<-gam(log(flow_out_mean)~s(r3score,bs="cr")+s(log(gdp),bs="cr"),data=data_mean)
summary(mod_uni_gdp) #0.82  
shapiro.test(mod_uni_gdp$residuals) #normal
plot(mod_uni_gdp)
#gdp linear contribute
anova(mod_uni,mod_uni_gdp,test="F") #reject

mod_ted_edu<-gam(log(flow_out_mean)~s(ted,bs="cr")+s(edu_index,bs="cr"),data=data_mean)
summary(mod_ted_edu) #rsq 0.807
shapiro.test(mod_ted_edu$residuals) #normal
plot(mod_ted_edu)
anova(mod_ted,mod_ted_edu,test="F") #reject, edu is useful with ted

mod_uni_edu<-gam(log(flow_out_mean)~s(r3score,bs="cr")+s(edu_index,bs="cr"),data=data_mean)
summary(mod_uni_edu) #rsq 0.837
shapiro.test(mod_uni_edu$residuals) #normal
plot(mod_uni_edu)
#edu linear contribute, not significant
anova(mod_uni,mod_uni_edu,test="F") #reject

mod_uni_ted_gdp<-gam(log(flow_out_mean)~s(r3score,bs="cr")+s(ted,bs="cr")+s(log(gdp),bs="cr"),data=data_mean)
summary(mod_uni_ted_gdp) #rsq 0.925
shapiro.test(mod_uni_ted_gdp$residuals) #no normal
plot(mod_uni_ted_gdp)
anova(mod_uni_ted,mod_uni_ted_gdp,test="F")#0.05, limit , we can use reduced


mod_uni_ted_edu<-gam(log(flow_out_mean)~s(r3score,bs="cr")+s(ted,bs="cr")+s(edu_index,bs="cr"),data=data_mean)
summary(mod_uni_ted_edu) #rsq 0.925
shapiro.test(mod_uni_ted_edu$residuals) #no normal 
plot(mod_uni_ted_edu)
anova(mod_uni_ted,mod_uni_ted_edu,test="F")#no reject, we can use reduced


mod_uni_ted_l<-gam(log(flow_out_mean)~s(r3score,bs="cr")+ted,data=data_mean)
summary(mod_uni_ted_l) #rsq 0.916
shapiro.test(mod_uni_ted_l$residuals) #normal
plot(mod_uni_ted_l)
anova(mod_uni_ted_l,mod_uni_ted,test="F") #reject at 1%, nonlinear contribute useful

mod_uni_ted_df7<-gam(log(flow_out_mean)~s(r3score,bs="cr",k=7)+s(ted,bs="cr"),data=data_mean)
summary(mod_uni_ted_df7) #rsq 0.904
shapiro.test(mod_uni_ted_df7$residuals) #normal
plot(mod_uni_ted_df7)

mod_uni_ted_df6<-gam(log(flow_out_mean)~s(r3score,bs="cr",k=6)+s(ted,bs="cr",k=6),data=data_mean)
summary(mod_uni_ted_df6) #rsq 0.9
shapiro.test(mod_uni_ted_df6$residuals) #normal
plot(mod_uni_ted_df6)

anova(mod_uni_ted_df7,mod_uni_ted,test="F") #reject, cant reduce
anova(mod_uni_ted_df6,mod_uni_ted,test="F") #reject, cant reduce
anova(mod_uni_ted_df6,mod_uni_ted_df7,test="F") #if we decide to smooth we can smooth everithing 
...
with(data_mean, points3d(r3score,ted,log(flow_out_mean+1),col='black',size=5))


## backward selection with linear ted
plot(data_mean$ted,log(data_mean$flow_out_mean))
plot(mod_ted)
summary(mod_ted) #rsq 0.728

mod9<-gam(log(flow_out_mean)~s(r3score,bs="cr")+s(log(gdp),bs="cr")+ted+s(edu_index,bs="cr"),data=data_mean)
summary(mod9)
shapiro.test(mod9$residuals)

mod10<-gam(log(flow_out_mean)~s(r3score,bs="cr")+s(log(gdp),bs="cr")+ted,data=data_mean)
summary(mod10)
shapiro.test(mod10$residuals)

mod11<-gam(log(flow_out_mean)~s(r3score,bs="cr")+ted,data=data_mean)
summary(mod11)
shapiro.test(mod11$residuals)

anova(mod11,mod9,test="F")

mod12<-gam(log(flow_out_mean)~s(r3score,bs="cr",k=6)+s(ted,bs="cr"),data=data_mean)
summary(mod12)
