...
library(readr)
country_years <- read_csv("D:/Marta/Politecnico/Tesi/Analisi nazionali/country_years.csv")

level=0
nuts <- SimDesign::quiet(eurostat::get_eurostat_geospatial(nuts_level = level,year="2021"))
...
countries<-unique(country_years$country)
idx<-setdiff(data$NUTS_0_code,countries)
setdiff(countries,data$NUTS_0_code)

data<-data[-which(data$NUTS_0_code %in% idx),]

idx2<-setdiff(colnames(data),countries)
idx2<-idx2[-1]
data<-data[,-which(colnames(data) %in% idx2)]

#srotoloooo
n<-dim(data)[1]
sender<-rep(data$NUTS_0_code,n)
...
write.csv(data_col,"countries_distances.csv",row.names = F)
